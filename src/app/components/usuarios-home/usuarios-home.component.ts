...
obtenerUsuarios(): void {
  ...
  this.usuariosService.getUsuarios(this.paginaActual).subscribe({
    next: (response: any) => {
      ...
      // Paginación igual a voluntarios
 ...
 cargarRoles(): void {
  this.usuariosService.getRoles().subscribe({
    next: (roles) => {
      ...
      console.log('Roles disponibles:', this.rolesDisponibles); // ✅ AHORA SÍ
    },
        ...
        next: (rolesRaw: any[]) => {
          ...
          Swal.fire({
            ...
            preConfirm: () => {
              ...
              if (!name || !last_name || !email || !password || !state_id || roles.length === 0) {
                Swal.showValidationMessage("Por favor completa todos los campos obligatorios (*)");
                return false;
              }
