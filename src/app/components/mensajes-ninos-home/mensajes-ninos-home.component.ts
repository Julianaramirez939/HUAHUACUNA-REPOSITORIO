...
export class MensajesNinosHomeComponent implements OnInit {
  ...
  abrirModalCrearMensaje(): void {
    ...
    Swal.fire({
      ...
      preConfirm: () => {
        const padrino_id = Number((document.getElementById("padrinoSelect") as HTMLSelectElement).value);
        const children_id = Number((document.getElementById("childrenSelect") as HTMLSelectElement).value);
