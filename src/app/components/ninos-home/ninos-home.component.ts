...
export class NinosHomeComponent implements OnInit {
  ...
  constructor(private ninosService: NinosService, private constantesService: ConstantesService) {}
          ...
          Swal.fire({
            ...
            didOpen: () => {
              const selects = [
                ...
                document.getElementById('schoolGradeSelect') as HTMLSelectElement
              ];
            ...
            preConfirm: () => {
              ...
              const fathers_name = (document.getElementById("fathers_name") as HTMLInputElement).value.trim();
              const mothers_name = (document.getElementById("mothers_name") as HTMLInputElement).value.trim();
              const school_grade_id = Number((document.getElementById("schoolGradeSelect") as HTMLSelectElement).value);
              const likings = (document.getElementById("likings") as HTMLInputElement).value.trim();
              const additional_information = (document.getElementById("additional_information") as HTMLTextAreaElement).value.trim();
              ...
              if (!name || !last_name || !birth_date || !school_grade_id || !likings || !attachment || !state_id) {
      ...
      this.constantesService.obtenerGradosEscolares().subscribe({
        next: (gradosRaw: any[]) => {
          ...
          Swal.fire({
            title: `<span style="font-family: 'Segoe UI', sans-serif; font-weight:600; color:#003366;">Actualizar niño</span>`,
             html: `
              ...
              ">
                <div style="display:flex; flex-direction:column; gap:8px;">
                  <label><b>Nombre *</b></label>
                  <input id="name" type="text" class="swal-field" value="${nino.name}" required>

                  <label><b>Apellido *</b></label>
                  <input id="last_name" type="text" class="swal-field" value="${nino.last_name}" required>

                  <label><b>Fecha de nacimiento *</b></label>
                  <input id="birth_date" type="date" class="swal-field" value="${nino.birth_date}" required>
                ...
                <div style="display:flex; flex-direction:column; gap:8px;">
                  <label>Nombre del padre</label>
                  <input id="fathers_name" type="text" class="swal-field" value="${nino.fathers_name || ''}">

                  <label>Nombre de la madre</label>
                  <input id="mothers_name" type="text" class="swal-field" value="${nino.mothers_name || ''}">

                  <label><b>Gustos o intereses *</b></label>
                  <input id="likings" type="text" class="swal-field" value="${nino.likings}" required>
            ...
            cancelButtonColor: '#dc2626',
            didOpen: () => {
              const gradeSelectEl = document.getElementById('schoolGradeSelect') as HTMLSelectElement;
              const stateSelectEl = document.getElementById('stateSelect') as HTMLSelectElement;

              const gradoActual = grados.find(g => g.id === nino.school_grade || g.name === nino.school_grade_name);
            ...
            preConfirm: () => {
              ...
              if (!name || !last_name || !birth_date || !school_grade_id || !likings || !state_id) {
                Swal.showValidationMessage('Por favor completa todos los campos obligatorios (*)');
                return false;
              }
  ...
  Swal.fire({
    ...
    html: `
      ...
      ">
        ...
        <div>
          <strong>Grado escolar</strong><br> ${nino.school_grade_name ?? nino.school_grade}
        </div>

        <div style="grid-column: span 2;">
          <strong>Gustos o intereses</strong><br> ${nino.likings || 'No especificados'}
        </div>

        <div style="grid-column: span 2;">
          <strong>Información adicional</strong><br> ${nino.additional_information || 'Sin información adicional'}
        </div>
