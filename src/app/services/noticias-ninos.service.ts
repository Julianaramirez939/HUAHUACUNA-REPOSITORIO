...
export class NoticiasNinosService {
   ...
   */
 traerNoticias(page: number = 1): Observable<{ data: { reports: NinosNoticias[], pagination: any } }> {
  ...
  return this.http.get<{ data: any }>(url, { headers }).pipe(
    ...
    tap(() => console.log(`[NoticiasNinosService] Noticias obtenidas. Página ${page}`)),
...
obtenerNoticia(id: number): Observable<EditarNinosNoticias> {
  ...
  return this.http.get<{ data: EditarNinosNoticias }>(url, { headers }).pipe(
    map(resp => resp.data),
    tap(() => console.log(`[NoticiasNinosService] Noticia ${id} cargada para edición.`)),
    catchError(error => this.manejarError(error))
  );
}
  ...
  crearNoticia(noticia: CrearNinosNoticias): Observable<any> {
    ...
    return this.http.post<any>(this.endpoint, body, { headers }).pipe(
      tap(() => console.log(`[NoticiasNinosService] Noticia creada correctamente.`)),
      catchError(error => this.manejarError(error))
    );
  }
   ...
   */
  actualizarNoticia(id: number, noticia: ActualizarNinosNoticias): Observable<any> {
    const headers = this.getHeaders();
    const body = new FormData();
    ...
    return this.http.post<any>(url, body, { headers }).pipe(
      tap(() => console.log(`[NoticiasNinosService] Noticia ${id} actualizada correctamente.`)),
      catchError(error => this.manejarError(error))
    );
  }
