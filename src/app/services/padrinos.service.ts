...
export class PadrinoService {
  ...
  actualizarPadrino(id: number, data: ActualizarPadrino): Observable<any> {
    ...
    return this.http.post<any>(url, formData, { headers }).pipe(
      tap(() =>
        console.log(`[PadrinoService] Padrino ${id} actualizado correctamente.`)
      ),
      catchError((error) => this.manejarError(error))
    );
  }
  ...
  eliminarPadrino(id: number): Observable<any> {
    ...
    return this.http.delete<any>(url, { headers }).pipe(
      tap(() =>
        console.log(`[PadrinoService] Padrino ${id} eliminado correctamente.`)
      ),
      catchError((error) => this.manejarError(error))
    );
  }
  ...
  getPadrinos(page: number = 1): Observable<any> {
    ...
    return this.http.get<any>(url, { headers }).pipe(
      tap(() =>
        console.log(
          `[PadrinoService] Lista de padrinos obtenida. Página ${page}`
        )
      ),
      catchError((error) => this.manejarError(error))
    );
  ...
  traerPadrinosTodos(): Observable<Padrino[]> {
    ...
    return this.http.get<{ data: any }>(url, { headers }).pipe(
      ...
      tap((p) =>
        console.log(`[PadrinoService] Padrinos cargados: ${p.length}`)
      ),
      catchError((error) => this.manejarError(error))
    );
  }
  ...
  apadrinarNino(padrinoId: number, childrenIds: number[]): Observable<any> {
    ...
    return this.http.put<any>(url, body, { headers }).pipe(
      tap(() =>
        console.log(
          `[PadrinoService] Padrino ${padrinoId} apadrinó niños: [${childrenIds.join(
            ', '
          )}]`
        )
      ),
  ...
  quitarApadrinamientoNino(
    ...
    return this.http.put<any>(url, body, { headers }).pipe(
      tap(() =>
        console.log(
          `[PadrinoService] Padrino ${padrinoId} quitó apadrinamiento de los niños: [${childrenIds.join(
            ', '
          )}]`
        )
      ),
