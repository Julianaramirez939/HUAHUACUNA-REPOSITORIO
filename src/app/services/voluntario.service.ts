...
export class VoluntarioService {
  ...
  postularVoluntario(voluntario: Voluntario): Observable<any> {
    ...
    formData.append(
      'identification_type',
      voluntario.identification_type.toString()
    );
    ...
    return this.http.post<any>(this.endpoint, formData).pipe(
      tap(() =>
        console.log('[VoluntarioService] Postulación enviada correctamente.')
      ),
  ...
  actualizarVoluntario(voluntario: VoluntarioActualizar): Observable<any> {
    ...
    return this.http.put<any>(url, voluntario, { headers }).pipe(
      tap(() =>
        console.log(
          `[VoluntarioService] Voluntario ${voluntario.id} actualizado correctamente.`
        )
      ),
      catchError((error) => this.manejarError(error))
    );
  ...
  eliminarVoluntario(id: number): Observable<any> {
    ...
    return this.http.delete<any>(url, { headers }).pipe(
      tap(() =>
        console.log(
          `[VoluntarioService] Voluntario ${id} eliminado correctamente.`
        )
      ),
      catchError((error) => this.manejarError(error))
    );
  ...
  getVoluntarios(page: number = 1): Observable<any> {
    ...
    return this.http.get<any>(url, { headers }).pipe(
      tap(() =>
        console.log(
          `[VoluntarioService] Lista de voluntarios obtenida. Página ${page}`
        )
      ),
      catchError((error) => this.manejarError(error))
    );
