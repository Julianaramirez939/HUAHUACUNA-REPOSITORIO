...
export class ActividadesService {
  ...
  /**
   Metodo para traer las actividades paginadas
   */
  traerActividades(
    page: number = 1
  ): Observable<{ data: { programs: Actividad[]; pagination: any } }> {
    const token = sessionStorage.getItem('token') || '';
    const headers = new HttpHeaders({ Authorization: `Bearer ${token}` });
    const url = `${this.endpoint}?page=${page}`;

    return this.http
      .get<{ data: { programs: Actividad[]; pagination: any } }>(url, {
        headers,
      })
      .pipe(
        tap(() =>
          console.log(
            `[ActividadesService] Lista de actividades obtenida. Página ${page}`
          )
        ),
  ...
  traerTodasLasActividades(): Observable<Actividad[]> {
    ...
    return this.http
      .get<{ data: Actividad[][] }>(this.endpoint, { headers })
      .pipe(
        map((response) => response.data[0]),
        tap((actividades) =>
          console.log(
            `[ActividadesService] Lista completa de actividades obtenida. Total: ${actividades?.length}`
          )
        ),
        catchError((error) => this.manejarError(error))
      );
  ...
  crearActividad(actividad: Actividad): Observable<any> {
    ...
    if (actividad.state_id)
      formData.append('state_id', actividad.state_id.toString());
    ...
    if (actividad.program_type)
      formData.append('program_type', actividad.program_type.toString());
    if (actividad.price !== undefined && actividad.price !== null)
      formData.append('price', actividad.price.toString());
    if (actividad.observation)
      formData.append('observation', actividad.observation);
    if (actividad.attachment)
      formData.append('attachment', actividad.attachment);
    ...
    return this.http.post<any>(this.endpoint, formData, { headers }).pipe(
      tap(() =>
        console.log('[ActividadesService] Actividad creada correctamente.')
      ),
      catchError((error) => this.manejarError(error))
    );
  }
  ...
  actualizarActividad(
    ...
    if (actividad instanceof FormData) {
      ...
      if (actividad.program_type)
        body.append('program_type', actividad.program_type.toString());
      if (actividad.price !== undefined && actividad.price !== null)
        body.append('price', actividad.price.toString());
      if (actividad.observation)
        body.append('observation', actividad.observation);
      if (actividad.state_id)
        body.append('state_id', actividad.state_id.toString());
    ...
    return this.http.post<any>(url, body, { headers }).pipe(
      tap(() =>
        console.log(
          `[ActividadesService] Actividad ${id} actualizada correctamente.`
        )
      ),
      catchError((error) => this.manejarError(error))
    );
  ...
  eliminarActividad(id: number): Observable<any> {
    ...
    return this.http.delete<any>(url, { headers }).pipe(
      tap(() =>
        console.log(
          `[ActividadesService] Actividad ${id} eliminada correctamente.`
        )
      ),
      catchError((error) => this.manejarError(error))
    );
