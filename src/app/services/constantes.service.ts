...
export class ConstantesService {
  ...
  obtenerTiposIdentificacion(): Observable<any> {
    ...
    return this.http.get<any>(url).pipe(
      tap(() =>
        console.log(
          '[ConstantesService] Tipos de identificación obtenidos correctamente.'
        )
      ),
      catchError((error) => {
        console.error(
          '[ConstantesService] Error al obtener tipos de identificación:',
          error
        );
        const backendError = error?.error || {};
        const mensaje =
          backendError.message ||
          'No se pudieron obtener los tipos de identificación';
  ...
  obtenerTiposMetodoDonacion(): Observable<any[]> {
    ...
    return this.http
      .get<{ success: boolean; data: any[]; message: string }>(url)
      .pipe(
        map((res) => res.data),
        tap((tipos) =>
          console.log(
            '[ConstantesService] Tipos de método de donación obtenidos:',
            tipos
          )
        ),
        catchError((error) => {
          console.error(
            '[ConstantesService] Error al obtener tipos de método de donación:',
            error
          );
          const backendError = error?.error || {};
          const mensaje =
            backendError.message ||
            'No se pudieron obtener los tipos de método de donación';
  ...
  obtenerTiposActividad(): Observable<any[]> {
    ...
    return this.http
      ...
      .pipe(
        ...
        tap((tipos) =>
          console.log(
            '[ConstantesService] Tipos de actividad obtenidos:',
            tipos
          )
        ),
        catchError((error) => {
          console.error(
            '[ConstantesService] Error al obtener tipos de actividad:',
            error
          );
          const backendError = error?.error || {};
          const mensaje =
            backendError.message ||
            'No se pudieron obtener los tipos de actividad';
          return throwError(() => ({ message: mensaje }));
        })
      );
  }
  ...
  obtenerGradosEscolares(): Observable<GradoEscolar[]> {
    ...
    return this.http
      .get<{ success: boolean; data: GradoEscolar[]; message: string }>(url)
      .pipe(
        map((res) => res.data),
        tap((grados) =>
          console.log('[ConstantesService] Grados escolares obtenidos:', grados)
        ),
        catchError((error) => {
          console.error(
            '[ConstantesService] Error al obtener grados escolares:',
            error
          );
          const backendError = error?.error || {};
          const mensaje =
            backendError.message ||
            'No se pudieron obtener los grados escolares';
          return throwError(() => ({ message: mensaje }));
        })
      );
  }
