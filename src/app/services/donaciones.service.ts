...
export class DonacionesService {
  ...
  crearDonacion(donacion: CrearDonaciones): Observable<Donaciones> {
    return this.http
      .post<Donaciones>(this.endpoint, donacion, { headers: this.getHeaders() })
      .pipe(
        tap(() =>
          console.log('[DonacionesService] Donación creada correctamente.')
        ),
  ...
  actualizarDonacion(donacion: DonacionesActualizar): Observable<Donaciones> {
    ...
    return this.http
      .put<Donaciones>(url, donacion, { headers: this.getHeaders() })
      .pipe(
        tap(() =>
          console.log(
            `[DonacionesService] Donación ${donacion.id} actualizada correctamente.`
          )
        ),
        catchError((error) => this.manejarError(error))
      );
  ...
  eliminarDonacion(id: number): Observable<void> {
    ...
    return this.http.delete<void>(url, { headers: this.getHeaders() }).pipe(
      tap(() =>
        console.log(
          `[DonacionesService] Donación ${id} eliminada correctamente.`
        )
      ),
      catchError((error) => this.manejarError(error))
    );
  ...
  //Obtener las donaciones paginadas
  getDonaciones(page: number = 1): Observable<any> {
    ...
    return this.http.get<any>(url, { headers }).pipe(
      tap(() =>
        console.log(`[DonacionesService] Donaciones obtenidas. Página ${page}`)
      ),
      map((resp) => {
        return {
  ...
  generarReporte(reporte: Reporte): Observable<Blob> {
    ...
    return this.http
      ...
      .pipe(
        tap(() =>
          console.log('[DonacionesService] Reporte PDF generado correctamente.')
        ),
        catchError((error) => this.manejarError(error))
      );
  }
